...
class Passwd
{
    ...
    static public function password_hash($password, $algo, array $options = array())
    {
        ...
        switch ($algo) {
            case self::PASSWORD_BCRYPT:
                ...
                break;
            case self::PASSWORD_SHA512:
                if (version_compare(PHP_VERSION, '5.3.7', '>=')) {
                    trigger_error("The Password Compatibility Library requires PHP >= 5.3.7 (don't use SHA512 with this version : ".PHP_VERSION.")", E_USER_WARNING);
                }
                // Note that this is a C constant, but not exposed to PHP, so we don't define it here.
                $cost = 5000;
                if (isset($options['cost'])) {
                ...
                $required_salt_len = 22;
                $hash_format = sprintf('$6$rounds=%d$', $cost);
                break;
            default:
        ...
        if (!is_string($ret) || ($algo === self::PASSWORD_BCRYPT && strlen($ret) <= 13) || ($algo === self::PASSWORD_SHA512 && strlen($ret) <= 117)) {
            return false;
        }
    ...
    static public function password_get_info($hash)
    {
        ...
        } elseif (substr($hash, 0, 3) == '$6$' && strlen($hash) == 60) {
            $return['algo'] = self::PASSWORD_SHA512;
            $return['algoName'] = 'sha512';
            list($cost) = sscanf($hash, "$6$round=%d$");
            $return['options']['cost'] = $cost;
        }
